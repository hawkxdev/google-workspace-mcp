...
def test_cross_service_token_isolation(
    ...
    g_res = 'https://127.0.0.1:8431'
    d_res = 'https://127.0.0.1:8433'
...
def test_all_five_services_multi_isolation_and_route_collisions(
    ...
    try:
        ...
        for (srv_name, port), app, state in zip(
            ...
            reg = state.register_client(('https://127.0.0.1/callback',))
            tok = state.issue_access_token(
                client_id=reg.client.client_id,
                resource=f'https://127.0.0.1:{port}',
            )
            ...
            res_res_meta = client.get('/.well-known/oauth-protected-resource')
            ...
            res_as_meta = client.get('/.well-known/oauth-authorization-server')
            assert res_as_meta.status_code == 200
            assert (
                res_as_meta.json().get('issuer') == f'https://127.0.0.1:{port}'
            )
...
def test_extension_route_collisions_and_isolation(
    ...
    try:
        ...
        reg_std = state.register_client(('https://127.0.0.1/callback',))

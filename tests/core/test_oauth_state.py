...
SERVICE = 'gmail'
READONLY = ('gmail_search', 'gmail_get_message')
...
def state_paths(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    return downloads, state_path, legacy_path
    ...
    *,
    ...
    state = OAuthState(
        ...
        static_client_authenticator=lambda client_id, secret: (
            ...
            and secrets_equal(secret, static_secret)
        ),
    )
...
def test_versioned_schema_and_owner_only_files_survive_reopen(state_paths):
...
def test_state_path_must_be_outside_download_dir(tmp_path):
    downloads = tmp_path / 'downloads'
    downloads.mkdir(mode=0o700)

    with pytest.raises(UnsafeStatePath, match='outside'):
        OAuthState(
            downloads / 'oauth.sqlite3',
            download_path=downloads,
            service_id=SERVICE,
            resource=RESOURCE,
            readonly_capabilities=READONLY,
...
def test_unsafe_mode_and_foreign_owner_are_rejected(state_paths, monkeypatch):
    ...
    import google_workspace_mcp.auth.state as oauth_state_module

    actual_uid = os.getuid()
    monkeypatch.setattr(
        oauth_state_module.os, 'getuid', lambda: actual_uid + 1
    )
...
def test_registration_stores_only_domain_separated_verifiers(state_paths):
    ...
    assert state.verify_client_secret(
        first.client.client_id, first.client_secret
    )
...
def test_authorization_code_expires_at_300_seconds_and_is_single_use(
    state_paths,
):
    now = [5_000.0]
    state = _state(state_paths, now=now)
...
def test_code_exchange_checks_client_secret_redirect_pkce_and_resource(
    state_paths,
):
    state = _state(state_paths)
    client_id, secret = _register(state)
...
def test_client_revoke_cascades_codes_and_tokens_across_reopen(state_paths):
    ...
    assert (
        reopened.lookup_access_token(second_token.access_token)
        == second_token.token
    )
    reopened.close()
...
def test_legacy_migration_is_allowlisted_one_way_and_secret_free(state_paths):
    ...
    assert (
        state.get_client('pending-client').policy == REAUTHORIZATION_REQUIRED
    )
    ...
    assert transitioned.policy == MCP_READONLY_V1
    assert transitioned.capabilities == READONLY
    assert 'approved-secret-marker' not in _db_text(state_path)
    assert 'pending-secret-marker' not in _db_text(state_path)
    state.close()


def test_two_registrations_of_one_connector_share_a_redirect(state_paths):
    _, state_path, legacy_path = state_paths
    records = {
        'connector-first': {
    ...
    state = _state(state_paths, approved=frozenset({'connector-second'}))

    assert not legacy_path.exists()
    assert state.get_client('connector-first').policy == (
        REAUTHORIZATION_REQUIRED
    )
    assert state.get_client('connector-second').policy == LEGACY_FULL
    assert state.get_client('connector-first').redirect_uris == (REDIRECT,)
    assert state.get_client('connector-second').redirect_uris == (REDIRECT,)
    assert 'first-secret-marker' not in _db_text(state_path)
    assert 'second-secret-marker' not in _db_text(state_path)
    state.close()


def test_one_client_repeating_a_redirect_uri_is_rejected(state_paths):
    _, state_path, legacy_path = state_paths
    _write_legacy(
        legacy_path,
        {
    ...
    with pytest.raises(
        ValueError, match="legacy client 'solo' has duplicate redirect URIs"
    ):
        _state(state_paths)
    ...
    if state_path.exists():
        with sqlite3.connect(state_path) as connection:
            clients = connection.execute('SELECT count(*) FROM clients')
            assert clients.fetchone()[0] == 0
...
def test_backup_rejects_unsafe_target_and_produces_reopenable_copy(
    ...
    with sqlite3.connect(backup) as connection:
        ...
        assert (
            connection.execute('SELECT count(*) FROM clients').fetchone()[0]
            == 1
        )
    ...
    with pytest.raises(UnsafeStatePath, match='outside'):
        state.backup(downloads / 'backup.sqlite3')
    state.close()
